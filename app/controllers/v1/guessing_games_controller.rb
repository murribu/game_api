module V1
  class GuessingGamesController < ApplicationController
    ...
    def start
      ...
      guessing_game_instance = guessing_game.guessing_game_instances.includes(:guessing_game_instance_option_values).create(:user_id => @auth_user.id)
      guessing_game.guessing_game_options.each do |o|
        if params[:options] && params[:options][o.id.to_s]
          ...
          guessing_game_instance.guessing_game_instance_option_values.create(:guessing_game_option_value_id => i)
        end
      end
      render json: guessing_game_instance.to_json(:include => :guessing_game_instance_option_values)
    end
  end
end